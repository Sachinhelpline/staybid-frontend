                ...
                {tickets.map((t) => (
                  <button key={t.id} onClick={() => setOpenId(t.id)} style={{ ...card, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, padding: "12px 16px", textAlign: "left", cursor: "pointer" }}>
                    <div style={{ minWidth: 0 }}>
                      <p style={{ fontWeight: 500, color: "#222", margin: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                      ...
                      <p style={{ fontSize: 12, color: "#888", margin: "2px 0 0" }}>{t.category ? `${t.category} · ` : ""}{fmt(t.updatedAt)}{t.priority === "high" ? " · 🔴 Urgent" : ""}{t.unread ? " · 🟢 New reply" : ""}</p>
                    </div>
                    {pill(t.status)}
                  </button>
                ))}
