...
// Shared metrics dashboard used by BOTH /admin/support/metrics and /agent/metrics.
...
export type SupportMetricsProps = {
  tokenKey: string;
  userKey: string;
  backHref: string;
  /** When true, hides the inline H1 title (caller already shows page chrome). */
  hideTitle?: boolean;
};
...
export default function SupportMetrics({ tokenKey, userKey, backHref, hideTitle = false }: SupportMetricsProps) {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`/api/admin/support/metrics?days=${days}`, { headers: buildHeaders(tokenKey, userKey) })
      .then((r) => (r.ok ? r.json() : null))
      .then((j) => setData(j))
      .finally(() => setLoading(false));
  }, [days, tokenKey, userKey]);
      ...
      <header style={styles.header}>
        <div>
          <div style={styles.eyebrow}>
            <Link href={backHref} style={styles.backLink}>← Support Inbox</Link>
          </div>
          {!hideTitle && (
            <>
        ...
        <div style={styles.rangeTabs}>
          {([7, 30, 90] as const).map((d) => (
            <button
              ...
              type="button"
              onClick={() => setDays(d)}
              style={{ ...styles.rangeTab, ...(days === d ? styles.rangeTabActive : {}) }}
            >
              {d}d
            </button>
          ))}
      ...
      {loading || !data ? (
        ...
        <div style={styles.body}>
          <section style={styles.kpiGrid}>
            <Kpi label="Conversations" value={data.summary.total} sub={`last ${days} days`} />
            <Kpi
              label="AI deflected"
            ...
            <Kpi
              ...
              sub={`${data.summary.total > 0 ? Math.round((data.summary.humanTouched / data.summary.total) * 100) : 0}% of total`}
              accent="#FF4757"
            />
            <Kpi
              ...
              accent="#2ECC71"
            />
            <Kpi label="Avg resolution" value={formatDuration(data.summary.avgResolutionMs)} sub="from first message" />
            <Kpi
              label="Live queue"
          ...
          <section style={styles.chartCard}>
            ...
            <ResponsiveContainer width="100%" height={260}>
              <LineChart data={data.dailyCounts} margin={{ top: 10, right: 14, bottom: 10, left: -10 }}>
                ...
                <XAxis dataKey="date" stroke="#8A8FA8" tickFormatter={(d) => d.slice(5)} fontSize={11} />
  ...
  return (
    <div style={{ marginBottom: 14 }}>
      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "#E8EAF0", marginBottom: 5 }}>
        <span>{label}</span>
        <span style={{ color: "#8A8FA8" }}>{value} ({Math.round(pct)}%)</span>
      </div>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  backLink: { color: "#8A8FA8", fontSize: 12, textDecoration: "none" },
  title: { fontFamily: "'Syne', serif", fontSize: 24, fontWeight: 700, margin: 0, color: "#E8EAF0" },
  ...
  kpi: { background: "#151820", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 12, padding: 14 },
  kpiLabel: { fontSize: 11, color: "#8A8FA8", textTransform: "uppercase", letterSpacing: 0.05, fontWeight: 600 },
  kpiValue: { fontSize: 24, fontWeight: 700, margin: "6px 0", fontFamily: "'Syne', serif" },
  kpiSub: { fontSize: 11, color: "#8A8FA8" },
  chartCard: { background: "#0F1117", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 14, padding: 16 },
  chartTitle: { fontSize: 14, fontWeight: 600, margin: "0 0 14px 0", color: "#E8EAF0" },
  twoCol: { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(380px, 1fr))", gap: 12 },
  ...
  th: {
    textAlign: "left", fontSize: 11, fontWeight: 700, color: "#8A8FA8",
    textTransform: "uppercase", letterSpacing: 0.05, padding: "8px 8px",
    borderBottom: "1px solid rgba(255,255,255,0.07)",
  },
  thRight: {
    textAlign: "right", fontSize: 11, fontWeight: 700, color: "#8A8FA8",
    textTransform: "uppercase", letterSpacing: 0.05, padding: "8px 8px",
    borderBottom: "1px solid rgba(255,255,255,0.07)",
  },
