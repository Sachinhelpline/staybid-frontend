...
// Shared inbox UI used by BOTH /admin/support and /agent.
// Differs only in which localStorage keys carry the auth token + identity
// + where the Metrics link points.
//
// All API calls go through /api/admin/support/* — those routes accept
// any role in (admin | super_admin | support_agent), so the same
// endpoints serve both panels.
...
export type SupportInboxProps = {
  tokenKey: string; // e.g. "sb_admin_token" or "sb_agent_token"
  userKey: string;  // e.g. "sb_admin_user" or "sb_agent_user"
...
export default function SupportInbox({
  ...
  return (
    <div style={{ ...styles.root, height: hidePageTopbar ? "100%" : "calc(100vh - 60px)" }}>
      {!hidePageTopbar && (
...
const styles: Record<string, React.CSSProperties> = {
  ...
  listScroll: { flex: 1, overflowY: "auto", display: "flex", flexDirection: "column", gap: 8 },
  empty: { padding: 28, textAlign: "center", color: "#8A8FA8", fontSize: 13 },
  convItem: {
    background: "#151820",
    border: "1px solid rgba(255,255,255,0.07)",
    color: "#E8EAF0",
  ...
  convItemMeta: { fontSize: 11.5, color: "#8A8FA8", lineHeight: 1.4 },
  convItemAgent: { fontSize: 11, color: "#7BA361", marginTop: 2, fontWeight: 600 },
  unreadBadge: {
    background: "#FF4757",
    color: "#fff",
    fontSize: 11,
  ...
  detailScroll: { flex: 1, overflowY: "auto", padding: 20, background: "#07080C" },
  composer: {
    flex: "0 0 auto",
    padding: 16,
    background: "#0F1117",
  ...
  composerTools: { display: "flex", gap: 8, alignItems: "center" },
  toolBtn: {
    background: "#151820",
    color: "#E8EAF0",
  ...
  contextInner: { padding: 18, display: "flex", flexDirection: "column", gap: 16 },
  contextSection: {
    background: "#151820",
    border: "1px solid rgba(255,255,255,0.07)",
    borderRadius: 10,
