...
- **🔒 Google admin sign-in is a VERIFIED exchange + admin intent FAILS CLOSED (v622):**
  ...
  - **Claim minimization:** the `social-login` POST body is `{ idToken }` ONLY — no
    ...
    client-supplied identity. **Admin intent forces `prompt: select_account`** on the Google provider
...
## Current production state — LIVE SESSION RECORD

**📋 AUTHORITATIVE CURRENT STATE:** See `docs/upgrade/99-PROGRESS-LEDGER.md` for the live session record. That ledger is updated after EVERY work session/PR merge and contains:
- Latest version badge + release date
- What shipped in the current session
- Gates passed (tsc · build · test:security · migrations verified)
- Known blockers / follow-ups for the next session

**This CLAUDE.md section is HISTORICAL GUIDANCE ONLY.** If you see a version reference here (e.g., "v621", "v571") and the ledger shows a newer version (e.g., "v733"), the ledger is authoritative. Real current state is: `git status`, git log, and the latest ledger entry.
