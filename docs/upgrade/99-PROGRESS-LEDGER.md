...
### 2026-08-15 — Classic reservation atomic pre-payment RPC + abandoned-pending expiry + concurrency suite (v734) — PR #550 (draft)
**Closes the classic (unit-less) M4 inventory oversell race documented across phases 1–4C. Two concurrent buyers on the same category could both pass the non-atomic `classicCategoryFree` read and both mint pending blocks; the invariant `successful reservations ≤ rooms.quantity` could fail under load. tsc + build clean, `test:security` 385/0, new `test:concurrency` 16/0.**
- **Migration `migrations/2026-08-14-v734-classic-reservation-rpc.sql`** — two new PL/pgSQL RPCs, additive-only (no schema changes to any existing table):
  - `reserve_classic_block(...)` — serialises callers on `(hotelId, roomId)` with `pg_advisory_xact_lock`; idempotent same-blockId short-circuit **inside** the lock (never a duplicate-PK 500); peak-occupancy union of `room_blocks` + hard-blocking bids (`ACCEPTED/COUNTER/CONFIRMED/CHECKED_IN` × `numRooms`) mirroring `lib/availability.ts::getOccupations` **exactly** (PENDING excluded per v247); strict `invhold_<blockId>` shape check that 409s on any mismatch instead of silent merge; INSERTs both the pending `inventory_blocks` row **and** the deterministic `invhold_<blockId>` `room_blocks` hold in one transaction under the lock. Returns `jsonb {ok, code, ...}`.
  - `assert_classic_hold_still_ours(text)` — pure-read helper the classic verify path now uses **instead of** `writeHold`'s upsert, so a released hold is never silently recreated post-payment (which would reintroduce the race the RPC was designed to close). Applied live to Supabase project `uxxhbdqedazpmvbvaosh` as migration version `20260815050744`; live signature check confirms both functions with correct args/returns/volatility/comment; no existing table columns altered.
- **`lib/inventory/assign.ts`** — new `reserveClassicAtomically` + `assertClassicHoldStillOurs` wrappers over the RPCs. `classicCategoryFree` kept in place, flagged deprecated per the additive rule.
- **Checkout rewires** (`app/api/b2b/listings/[id]/checkout`, `app/api/b2b/basket/checkout`) — classic branch calls `reserveClassicAtomically`; the pre-flight `classicCategoryFree` gate is removed (the RPC is the single gate). `category_full` / `no_capacity_signal` → 409 **pre-payment**, so the browser never opens the Razorpay widget and there's no captured payment to refund. Unit branch (successful `assignFreeUnit`) unchanged.
- **Verify rewires** (`app/api/b2b/listings/[id]/verify`, `app/api/b2b/basket/verify`) — `writeHold` branches on `hasUnit`. Unit branch keeps its existing `merge-duplicates` upsert. Classic branch asserts the invhold still matches; a missing or shape-drifted hold flips `{held:false, needsOpsReview:true}`, stamps `metadata.holdLostAtVerify` on the block, but **never** recreates the row. Payment is captured either way (bookkeeping proceeds); the ops surface reconciles.
- **Abandoned-pending expiry** (`app/api/cron/inventory-lifecycle`) — new **Pass 5** `abandonedPendingPass`: `PENDING_TTL_MINUTES = 30` (matches `ACCEPTED_UNPAID_WINDOW_MIN` from `lib/bid-expiry.ts`). Guarded PATCH `pending_payment → expired` on `inventory_blocks` + `releaseHold(id)` via the existing helper. Idempotent. Runs **inside** the existing `runAll` execution — **no new cron registration**; the cron-job.org `*/15` schedule that already drives `/api/cron/inventory-lifecycle` picks it up.
- **M1 marketplace intentionally NOT touched** — `host_circle` properties are unit-only by design; checkout hard-stops with 409 when `assignFreeUnit` returns null, and verify always stamps `assignedUnitId` from `block.unit_id`. No classic branch exists to rewire.
- **New automated concurrency suite** (`tests/concurrency/`, run via `npm run test:concurrency`) — throwaway PG-16 cluster on a socket-only harness with DSN guard. Real parallel `pg.Client` sessions (never a shared pool). 16 tests exercise A–I from phase 5: A/B/C capacity=1/N wins + rest 409, D non-overlap both succeed, E same-blockId idempotent, F.walk_in/F.ota_ical/F.manual/F.inventory every source counted, G hard-blocking bids × `numRooms` counted like `getOccupations`, H PENDING uncounted, I.1–I.5 hold pre-existence / verify no-second-write / missing-hold never recreated / abandoned-expiry release / verify ⇄ expiry race in both orderings. Every test asserts final DB row state, not just RPC returns. `pg` added as devDep only.
- **Ship-checklist rules applied:** step 1 — SB_BUILD `v733-…` → `v734-classic-reservation-atomic-rpc` + visible badge `v733` → `v734`. Step 2 — `public/sw.js HTML_CACHE` **not bumped** (server-only / pure-logic ship, no UI/HTML change; stays at `staybid-html-v526`).
- ⚠ **PR #550 is DRAFT — not merged.** Live Supabase migration applied via `mcp__Supabase__apply_migration` before any code merges to `main`, so a pre-merge Vercel preview and any Production traffic already have the RPCs available (`CREATE OR REPLACE FUNCTION`, always-additive, safe). Owner review + explicit "ready for review" / merge instruction still pending.
...
### 2026-08-04 — Partner: new "Manage My Price" hub tab + high-tech Sell-to-Agents redesign (v730)
**Owner (2 screenshots): "no price control button on the partner dashboard" + "the Publish-a-new-lot card is flat — make it high-tech/modern, visually + usability." Owner chose: a dedicated price-hub tab + redesign the WHOLE Sell-to-Agents tab. UI-only (+ one additive API field); tsc 0 · build 0 · `test:security` 385/0.**
- **Root cause of "no price control":** an investigation confirmed there was NO button/tab/route labelled price/pricing anywhere. Per-room floor/flash editing was buried inside the **Rooms** tab (which scrolls off-screen, left of "F&B Menu", in the owner's 24-tab bar); v727's "Manage My Price" shipped only as a read-only *"How your price works"* explainer, never a real surface.
- **NEW `💰 Manage My Price` tab** (`app/partner/dashboard/page.tsx`) — placed **2nd** (right after Overview) so price control is discoverable without scrolling, + a prominent Overview quick-launch tile. A modern price hub that REUSES the existing state/engine (no new pricing engine): per-room **price-ladder gauge** (Bid Floor → **AI live price** gold marker → Rack/MRP), editable **Bid Floor / Rack-MRP / Flash Floor** (₹-prefixed `inp-p`), "Apply AI" quick-fills, Save (`saveRoomPricing`), a "How your price works" explainer with a mini ladder legend, and channel-launcher chips (🏷️ Travel agents · 🔗 OTA · 🛏️ B2B units). Wired: TABS entry + `PICON.pricing=IndianRupee` + `STAFF_TAB_ALLOW.manager` + tab-union type + `editPrices` gains `mrp` + Overview tile.
- **Rack/MRP now editable end-to-end (additive):** `app/api/rooms/[id]/pricing/route.ts` PUT accepts an optional `mrp` (NaN/≤0-guarded so a stray value never wipes it); `saveRoomPricing` sends it. Legacy callers (Rooms tab, floor/flash only) are byte-identical.
- **Sell-to-Agents tab fully redesigned** (`components/partner/AgentAuctionTab.tsx`) — **presentation only; all state/effects/handlers (`publish`/`cancelLot`/`actOnBid`/`loadLots`/`loadPending`), derived floors (`effFloor`/`propFloorPreview`/circle-vs-property branch), and every fetch are byte-identical to v729.** New look: a dark steel HERO with a live badge + trust chips; a premium sectioned "Publish a new lot" card (elevated Live/Sealed + autopilot tiles with active steel-glow, a **−/＋ rooms stepper**, ₹-prefixed inputs, a **live wholesale-floor GAUGE** — Safety Anchor → Your-Floor gold marker → Market ADR — over the existing discount slider, modern info pills, a prominent full-width `btn-gold` CTA); premium live-bids review + "Your lots" list with animated status dots. Matches the dashboard design system (`.card-p`/`.inp-p`/`.btn-gold` steel-gradient + gold accents) and the hub's ladder motif.
- **Gates:** tsc 0 · build 0 · `test:security` **385/0**. Adversarial review workflow (3 lenses → verify) run over both surfaces. Badge v729→**v730** (`v730-manage-my-price-hub`), sw HTML_CACHE v522→**v523** (partner-dashboard markup changed).

### 2026-08-04 — Admin: universal dead-space fix (full-width) + high-tech Pricing Engine redesign (v731)
**Owner (2 screenshots of the admin Pricing Engine): a big dead gap between the left menu and the content on EVERY admin section (left AND right) — make each page fill the whole screen (up to ~2560px), not centered; and the Pricing-Engine cards are old/ugly/hard-to-use — make them modern/high-tech. UI-only; tsc 0 · build 0 · `test:security` 385/0.**
- **Root cause of the dead space (code-verified, universal):** `app/admin/layout.tsx` `<main>` had BOTH `marginLeft: sideW` (240px, already clears the fixed sidebar) AND `paddingLeft: sideW + 32` (272px) — the sidebar width was **double-counted**, pushing content ~272px past the sidebar (a dead gap on **every** admin page). Fixed to a normal 32px gutter (`padding: "80px 32px 32px 32px"`), so content now fills full width after the sidebar, edge-to-edge, scaling to large monitors. Removed the dead `paddingTop:64` (the shorthand already set 80).
- **Per-page centering caps removed** (they'd still center content in the now-full width): `app/admin/kpi`, `app/admin/auction` (Agent Auction admin), `app/admin/passport` each dropped their `maxWidth ~1080/1100 + margin:0 auto`. Modal/drawer/table-cell `maxWidth`s were left intact (intentional).
- **Pricing Engine redesigned modern/high-tech** (`components/admin/PricingEngineTab.tsx`) — **presentation only; all state/effects/API calls byte-identical.** Removed the `maxWidth:1080` cap (fills full width; the month/day/occupancy grids now breathe into more columns). New look via a `.pe-*` class set in `app/admin/admin.css`: elevated section cards with an **icon + accent header**, NumField "control cards" in a responsive grid (₹-less numeric inputs with a **unit suffix** ×/% and an elegant `def`/changed indicator), styled **multiplier tiles** (months/DoW/occupancy/lead/festival/city — changed→gold ring), a **segmented** Static/Dynamic control, modern toggles, and a **sticky glass save bar** (gold primary + ghost reset). Dark admin theme (Syne/DM Sans; gold/blue/green/purple/amber accents), hover/focus transitions.
- **Folded in 3 confirmed findings from the v730 adversarial review** (all in the just-shipped "Manage My Price" hub): (1) MEDIUM a11y — the 3 price inputs had no accessible name → added `aria-label` (Bid Floor / Rack-MRP / Flash floor, per room); (2) LOW — the ladder used `??` so clearing a field flashed ₹0 mid-edit → switched to `||` (empty string falls back to the saved value; explicit "0" still respected); (3) LOW/cosmetic (AI marker can sit outside the floor↔MRP band in rare data) — left as-is (position already clamped 7–93%; changing the displayed number would mislead), documented.
- **Gates:** tsc 0 · build 0 · `test:security` **385/0**. Badge v730→**v731** (`v731-admin-fullwidth-pricing-engine`), sw HTML_CACHE v523→**v524** (admin markup changed).

### 2026-08-04 — /bid: step-sheet CTA + content never clipped behind the bottom dock (v732)
**Owner (3 screenshots, steps 1/2/5): on the bid page every step's window clips its content and the CTA button (Done / Launch Bid) is half-hidden behind the bottom nav dock — "zero hide, zero overlapping, 100% perfect, every device." UI-only; tsc 0 · build 0 · `test:security` 385/0.**
- **Root cause (code-verified):** the `.cmm-sheet` step-sheet backdrop (`z-1080`) renders INSIDE `.bgz-shell` (`components/ClimberMilestoneMap.tsx`), and `.bgz-shell` sets **`isolation: isolate`** (a z-1000 stacking context). So the sheet's `z-1080` is trapped at the shell's `z-1000` root level — **BELOW the /bid dock's `z-1001`** (`body.sb-bid-immersive .ig-bottom-dock`) — and the dock painted OVER the sheet's CTA + bottom content on **every** step. (The date step happened to be OK because `LuxuryCalendar` already sets `sb-modal-open`; the other 6 steps didn't.)
- **Fix (minimal, consistent, bulletproof):** the step sheet is a MODAL, so wire its open-state into the app-wide **`sb-modal-open`** flag — the SAME one every other modal uses (`LuxuryCalendar`, `BookingReview`, feed modals, hotel page…), which already hides the dock via `body.sb-modal-open .ig-bottom-dock { display: none !important; }` (BottomDock). Extended the existing `sheetIdx` scroll-lock effect in `ClimberMilestoneMap.tsx` to add/remove `sb-modal-open`. `display:none !important` is absolute (device-independent — the dock is fully removed while a sheet is open, so no z-index can bring it back over the CTA); the sheet's own X handles escape, so users are never trapped, and the dock returns the instant the sheet closes (climber map keeps the dock, v614/v633 intent preserved). No z-index/layout/design alteration — the sheet's flex-column layout (head + scroll body + pinned CTA + `safe-area` padding) was already correct; it was only the dock overlap.
- **Every step covered:** the toggle keys on `sheetIdx !== null`, so all 7 steps (incl. auto-advance cards with no CTA, e.g. the city step whose AI-insight was clipped) get a fully-visible window on every device.
- **Gates:** tsc 0 · build 0 · `test:security` **385/0**. Badge v731→**v732** (`v732-bid-sheet-cta-visible`), sw HTML_CACHE v524→**v525** (/bid markup behavior changed).

### 2026-08-04 — Model-2 B2B wholesale price + partner full-bleed sign-in + Circle pricing in admin (v733)
**Owner (2 screenshots + 4 asks): (1) property-owner sign-in still a narrow centered column — make it full-bleed like the other sign-in pages; (2) Circle Model-2 B2B tour shows "buy price == market price" on almost every property → nobody would buy — deep-study the code, no blind assumptions, god-level future-proof fix; (3) merge Circle Model 1/2/3 pricing into the admin Pricing Engine; (4) the admin Pricing-Engine number-stepper arrows are black-on-dark (invisible). tsc 0 · build 0 · `test:security` 385/0.**
- **(#16) Partner sign-in full-bleed — ROOT CAUSE found by headless measurement:** `.pauth-root` computed to `max-width:480px; margin:0 auto` (a narrow centered column, x=480 at 1440px). Source = the global cap `main > div[class*="auth"] { max-width:480px; margin-inline:auto }` (`app/desktop.css`, ≥1024, layer `utilities`) — it catches `.p·auth·-root` because the class contains "auth". `/auth` escapes it via `.auth-root.lux-bg` (0,2,0 > 0,1,2); `/admin/login`'s root has NO className so it never matches; `/partner` had no escape. **Fix:** an UNLAYERED override in the page's own `<style>` — `.pauth-root { max-width:none; margin-inline:0; width:100% }` (unlayered beats the utilities layer regardless of specificity, exactly like `/auth`). Re-measured: full-bleed 1440/1024px (w=1440 x=0, maxW=none), full-width on mobile. (SSR confirmed the brand + form panes render; the client-side `null` earlier was a sandbox-only artifact — the blocked Google-Fonts `@import` breaks dev hydration here.)
- **(#15) B2B buy==market — ROOT CAUSE (deep code study + live SQL):** the `own × multiplier` price only makes sense when `own` is a REAL below-market acquisition cost (true for `investor_block` resale = the block's frozen buy cost, FALSE for `hotel_owner` supply = no purchase price). For hotel-owner supply the code substituted `own = the room's RETAIL FLOOR`, so `× 2` landed the buy AT/ABOVE the live market (`live_price` is floored at the retail floor). Demo catalogue: `own = floor/2, mult = 2 → buy = floor = market → 0% margin` (owner's exact example: Lakeview buy ₹6,400 == market ₹6,400, MRP ₹11,000). Live code path was even worse (`own = floor, × 2 → 2× floor`). **Fix (future-proof, mirrors the proven Model-3 wholesale floor):** hotel-owner supply is now priced as a genuine WHOLESALE DISCOUNT below the retail floor — `buy = snap100(retailFloor × (1 − wholesaleDiscount%))`, admin-tunable (default 25%). Because the retail floor is the lowest a guest ever pays, buy is ALWAYS below the live market → resale margin guaranteed ≥ the discount %, every season/property. New pure helper `wholesaleBuyPerNight` (`lib/b2b/engine.ts`); new `wholesale_discount_pct` knob in `b2b_fee_config` (config store + `/api/admin/b2b-fee`); `createHotelOwnerListing` + `regulated-quote` freeze `own = buy, multiplier = 1` (the markup model doesn't apply to hotel-owner supply); the investor-block resale path + the v725 owner-multiplier band are UNTOUCHED. Supply form (`CircleInventoryTab`) relabelled "Wholesale price · StayBid-regulated" (its multiplier slider auto-hides via min==max). Migration `2026-08-04-v733-b2b-wholesale-discount.sql` re-prices the 32 demo rows. **Live SQL round-trip (all 32): mult=1, own=ask, buy < floor (32/32), buy < live ADR (32/32), ZERO rows buy ≥ market, resale margin 24–37%.** Buyer display (`/circle/model2/[id]`) fixes itself — market-quote reads the re-frozen row → the "below market — resale upside" note now fires.
- **(#17) Circle pricing in the admin Pricing Engine:** a self-contained `<CirclePricingSections/>` appended to `components/admin/PricingEngineTab.tsx` (its OWN load + per-model save; the AI-formula save above is byte-identical). Model 1 = read-only reference tiles (12% resale + 12% guest-booking fees, code-set); Model 2 = wholesale discount / buyer fee / seller fee / resale multiplier / city access → `/api/admin/b2b-fee`; Model 3 = wholesale discount / circle floor × / buyer premium / seller fee / EMD deposit → `/api/admin/auction-config`. Additive UI over EXISTING routes — no contract change (b2b-fee POST just gained the optional `wholesaleDiscountPct`).
