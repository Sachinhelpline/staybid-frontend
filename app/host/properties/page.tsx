    ...
    <div className="hostos-discovery">
      <section className="max-w-7xl mx-auto px-4 sm:px-6 pt-10 pb-5 sb-fade-in">
        ...
        <div className="mt-4 flex items-center gap-3 flex-wrap">
          <Link href="/host/list-property" className="sb-card-lift inline-flex items-center gap-2 px-5 py-2.5 rounded-full font-semibold text-white shadow"
            style={{ background: "var(--accent)" }}>
            🏡 List your property for lease / rent →
          </Link>
          <span className="text-xs" style={{ color: "var(--text-muted)" }}>Own a property? Give it to StayBid to lease out.</span>
        </div>
      </section>
    ...
    <div className="sb-card-lift h-full rounded-2xl overflow-hidden flex flex-col"
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        <button onClick={onEnquire}
          ...
          style={{ background: "radial-gradient(88% 64% at 32% 4%,rgba(240,247,253,0.24),transparent 58%),linear-gradient(160deg,#a0b2c6 0%,#6f8aa6 50%,#42566d 100%)" }}>Enquire / Schedule visit</button>
      </div>
    </div>
  );
}
