...
const STATUS: Record<string, { label: string; color: string; bg: string }> = {
  ...
  na:  { label: "TRACKING",   color: "#a99a7c", bg: "rgba(217,190,130,0.08)" },
};
  ...
  return (
    <div style={{ padding: "26px 22px 60px", maxWidth: 1080, margin: "0 auto" }}>
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
        <div>
          <h1 style={{ fontFamily: '"Cormorant Garamond", serif', fontStyle: "italic", fontSize: "1.9rem", color: "#ffe9ad", margin: 0 }}>
            📊 KPI Scorecard
          </h1>
          <p style={{ fontSize: "0.78rem", color: "rgba(250,245,235,0.6)", margin: "4px 0 0" }}>
          ...
          {[30, 90, 180].map((d) => (
            <button
              ...
              style={{
                ...
                borderRadius: 999, border: "1px solid rgba(217,190,130,0.35)",
                background: days === d ? "linear-gradient(140deg,#ffe9ad,#f2c650 44%,#d69a1e)" : "rgba(217,190,130,0.08)",
                color: days === d ? "#3a2606" : "#f7ecd8",
              }}
            >
              {d}d
            </button>
      ...
      {err ? (
        ...
        <p style={{ marginTop: 26, color: "rgba(250,245,235,0.6)", fontSize: "0.85rem" }}>Loading…</p>
      ) : (
        <>
          <p style={{ margin: "18px 0 14px", fontSize: "0.72rem", color: "rgba(250,245,235,0.55)" }}>
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    border: "1px solid rgba(217,190,130,0.3)",
                    boxShadow: "inset 0 1px 0 rgba(255,233,173,0.14), 0 10px 22px -14px rgba(0,0,0,0.8)",
                  }}
                >
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
                    <span style={{ fontSize: "0.66rem", fontWeight: 700, letterSpacing: "0.05em", color: "rgba(250,245,235,0.65)", textTransform: "uppercase" }}>
                      {k.label}
                    </span>
                  ...
                  <div style={{ marginTop: 10, fontSize: "1.65rem", fontWeight: 800, color: "#fff3d6", fontVariantNumeric: "tabular-nums" }}>
                    {fmt(k)}
                  </div>
                  <div style={{ marginTop: 5, fontSize: "0.66rem", color: "rgba(255,233,173,0.75)" }}>
                    Target: {k.target}
                  </div>
                  {k.note ? (
                    <div style={{ marginTop: 6, fontSize: "0.6rem", color: "rgba(250,245,235,0.5)" }}>{k.note}</div>
                  ) : null}
                </div>
              );
            })}
          </div>
          <p style={{ marginTop: 18, fontSize: "0.62rem", color: "rgba(250,245,235,0.45)" }}>
