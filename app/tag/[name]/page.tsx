  ...
  if (!data || data.count === 0) {
    return (
      <div className="lux-bg inf-root min-h-screen flex flex-col items-center justify-center px-8 text-center">
        ...
        <Link href="/reels" className="btn-luxury sb-shimmer px-6 py-2 rounded-full mt-6 relative"><span className="relative" style={{ zIndex: 2 }}>Browse Reels</span></Link>
      </div>
    );
  }
      ...
      <div className="max-w-3xl mx-auto px-4 pt-6 pb-24">
        ...
        <div className="card-luxury sb-card-lift sb-fade-in p-6 relative overflow-hidden">
          ...
          <p className="text-luxury-600 text-sm mt-2">
            <span className="font-bold text-gold-700"><CountUp value={Number(data.count) || 0} duration={900} /></span> {data.count === 1 ? "reel" : "reels"}
          </p>
        ...
        {data.related?.length > 0 && (
          <div className="card-luxury sb-card-lift sb-fade-in p-5 mt-4" style={{ animationDelay: "0.1s" }}>
            <h2 className="text-xs uppercase tracking-widest font-bold text-luxury-500 mb-3">Related</h2>
            <div className="flex flex-wrap gap-2 sb-stagger">
              {data.related.map((r: any) => (
                <Link key={r.tag} href={`/tag/${encodeURIComponent(r.tag)}`}
                  className="px-3 py-1.5 rounded-full text-xs font-bold bg-luxury-100 text-luxury-700 hover:bg-gold-50 hover:text-gold-700 transition-colors sb-card-lift">
        ...
        {/* Reels grid */}
        <div className="card-luxury sb-card-lift sb-fade-in p-5 mt-4" style={{ animationDelay: "0.2s" }}>
          <h2 className="font-bold text-luxury-900 mb-4">Top reels</h2>
          <div className="grid grid-cols-3 gap-2 sb-stagger">
            {data.videos.map((v: any) => (
              <Link key={v.id} href={`/reels?tag=${encodeURIComponent(data.tag)}`}
                className="relative aspect-9/16 rounded-xl overflow-hidden bg-luxury-100 group sb-card-lift">
                {v.thumbnail_url
                ...
                <div className="absolute bottom-1.5 left-1.5 right-1.5 flex items-center justify-between text-white text-[0.6rem] font-bold drop-shadow-sm">
                  <span>▶ {fmtNum(v.views_count || 0)}</span>
                  <span>❤ {fmtNum(v.likes_count || 0)}</span>
                </div>
                {v.hotel && (
                  <div className="absolute top-1 left-1 right-1 text-white text-[0.55rem] font-bold drop-shadow-sm truncate">
                    🏨 {v.hotel.name}
                  </div>
                )}
              </Link>
