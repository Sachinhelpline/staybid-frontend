...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const [statsRes, commsRes] = await Promise.all([
    fetch(`${SB_URL}/rest/v1/influencer_stats?influencer_id=eq.${infId}&select=*`, { headers: SB_H }).then(r => r.json()).catch(() => []),
    fetch(`${SB_URL}/rest/v1/influencer_commissions?influencer_id=eq.${infId}&select=commission_amount,status,created_at`, { headers: SB_H }).then(r => r.json()).catch(() => []),
  ]);
  ...
  const pendingCommission = comms.filter(c => c.status === "pending").reduce((s, c) => s + Number(c.commission_amount || 0), 0);

  return NextResponse.json({
    stats: stats || { monthly_bookings: 0, monthly_commission: 0, total_followers: 0, content_count: 0 },
    derived: {
      totalBookings: comms.length,
