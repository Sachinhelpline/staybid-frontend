// HQ Support Desk — influencer surface. Raise + list tickets on the unified HQ store.
import { NextRequest, NextResponse } from "next/server";
import { creatorIdentity } from "@/lib/support/identity";
import { listMyTickets, createMyTicket } from "@/lib/support/desk";

export async function GET(req: NextRequest) {
  const id = await creatorIdentity(req);
