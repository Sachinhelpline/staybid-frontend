...
export async function GET(req: Request) {
  ...
  if (!user?.id) {
    return NextResponse.json({ signedIn: false, leads: [], inquiries: [], projects: [], orders: [], jobs: [], channels: [], summary: empty() });
  }
  const uid = encodeURIComponent(user.id);

  try {
    const [leads, inquiries, projects, orders, jobs, channels] = await Promise.all([
    ...
    const summary = {
      ...
      total: leads.length + inquiriesOut.length + projectsOut.length + ordersOut.length + jobsOut.length + channels.length,
    };

    return NextResponse.json({
      signedIn: true,
      leads, inquiries: inquiriesOut, projects: projectsOut, orders: ordersOut, jobs: jobsOut, channels,
      summary,
    });
  } catch (e: any) {
    return NextResponse.json({ signedIn: true, error: String(e?.message || e).slice(0, 160), leads: [], inquiries: [], projects: [], orders: [], jobs: [], channels: [], summary: empty() }, { status: 502 });
  }
}

function empty() {
  return { leads: 0, inquiries: 0, projects: 0, orders: 0, jobs: 0, channels: 0, storeSpend: 0, total: 0 };
}
