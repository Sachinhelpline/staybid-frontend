// HQ Support Desk — trade surface. Raise + list tickets on the unified HQ store.
import { NextRequest, NextResponse } from "next/server";
import { tradeIdentity } from "@/lib/support/identity";
import { listMyTickets, createMyTicket } from "@/lib/support/desk";

export async function GET(req: NextRequest) {
  const id = await tradeIdentity(req);
