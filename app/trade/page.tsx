...
export default function TradeBrowsePage() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(180deg,var(--trd-page-a),var(--trd-page-b))" }}>
      {/* Header */}
      {/* v713 (owner) — trade header was a hardcoded dark walnut bar on the light
          page; now a themed light bar (flips in dark) with a hairline base. */}
      <div className="sticky top-0 z-30" style={{ background: "var(--trd-card)", color: "var(--trd-ink)", borderBottom: "1px solid var(--trd-line)" }}>
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between gap-3">
          <div>
            <div className="text-xl font-semibold" style={{ color: "var(--trd-ink)", fontFamily: "var(--font-display, 'Cormorant Garamond', serif)" }}>StayBid Trade</div>
        ...
        {loading ? (
          ...
          <button key={l.id} onClick={() => router.push(`/trade/${l.id}`)}
            ...
            <div className="p-3.5 flex-1 flex flex-col">
              ...
              <div className="mt-3 w-full py-2.5 rounded-2xl font-bold text-white text-center" style={{ background: "radial-gradient(88% 64% at 32% 4%,rgba(240,247,253,0.24),transparent 58%),linear-gradient(160deg,#a0b2c6 0%,#6f8aa6 50%,#42566d 100%)" }}>
                View & bid
              </div>
            </div>
          </button>
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 pt-3">
      <div className="rounded-2xl p-4 flex items-center justify-between gap-3" style={{ background: "linear-gradient(135deg,#33251a,#4a3820)", color: "#ffe9c7" }}>
        ...
        <button onClick={goModel2} className="shrink-0 px-4 py-2 rounded-xl font-bold text-[0.8rem]" style={{ background: "radial-gradient(88% 64% at 32% 4%,rgba(240,247,253,0.24),transparent 58%),linear-gradient(160deg,#a0b2c6 0%,#6f8aa6 50%,#42566d 100%)", color: "#ffffff" }}>
          Open Model 2 →
        </button>
      </div>
    </div>
