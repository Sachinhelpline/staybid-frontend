...
// v357 — Circle Model 2 · Step 2 TOUR. A full property page (hotel-page replica
// for the B2B exchange), mirroring Model 1's /circle/[id]: hero gallery, badges,
// a metric grid, and "Choose your rooms" — each room opens its own LIVE
// availability calendar (a clean check-in → check-out range picker, every
// released night selectable) and a "market" panel (your buy price vs the room's
// real guest ADR / low / high, with resale upside). The internal own-price × N
// rule is NEVER shown — the buyer sees only what they pay + the market.
...
import { addItem, removeItem, readBasket, onBasketChange } from "@/lib/circle/model2-basket";

type Meta = {
  title?: string; city?: string; room?: string; capacity?: number; star?: number;
  description?: string; room_images?: string[]; prop_images?: string[]; amenities?: string[];
};
type Listing = {
  id: string; hotel_id: string; hotel_name: string | null; hotel_city: string; room_id: string;
  ask_per_night: number; date_from: string; date_to: string; metadata?: Meta | null;
};
type Quote = {
  window: { from: string; to: string; effectiveFrom: string };
  blocked: string[]; buyPerNight: number; market: { adr: number; low: number; high: number } | null;
...
function fc(l: Listing) {
  ...
  return {
    ...
    buyN: Number(l.ask_per_night) || 0, roomImgs: roomImgs.length ? roomImgs : propImgs,
    propImgs: propImgs.length ? propImgs : roomImgs,
  };
}
...
export default function Model2PropertyPage() {
  ...
  // headline market rate from the cheapest room (one call)
  useEffect(() => {
  ...
  if (authLoading || loading) {
    return <div className="sbc-home"><div className="sbc-ms-wrap"><div className="sbc-panel" style={{ padding: 30, textAlign: "center", color: "rgba(74,56,32,.6)" }}>Loading property…</div></div></div>;
  }
  if (!prop) {
    return <div className="sbc-home"><div className="sbc-ms-wrap"><Link href="/circle/model2/browse" className="sbc-ms-back" style={{ color: "var(--sbc-gold-deep)" }}>← Back</Link><div className="sbc-panel" style={{ padding: 24, marginTop: 12, color: "rgba(74,56,32,.6)" }}>This property has no released rooms right now.</div></div></div>;
  }
    ...
    <div className="sbc-home">
      <div className="sbc-ms-wrap sbc2p">
        ...
        <div className="sbc2p-hero">
          ...
          <div className="sbc2p-hero-cap"><div className="sbc2p-hero-title">{prop.title}</div><div className="sbc2p-hero-loc">📍 {cap(prop.city)}</div></div>
        </div>
        {imgs.length > 1 && (
          <div className="sbc2p-thumbs">{imgs.map((im, i) => <button key={i} className={`sbc2p-thumb${i === gi ? " on" : ""}`} onClick={() => setGi(i)}><img src={im} alt="" /></button>)}</div>
        )}

        {/* badges */}
        <div className="sbc2p-badges">
          {prop.star > 0 && <span className="sbc2p-badge">{"★".repeat(prop.star)}</span>}
          <span className="sbc2p-badge">🛎 Fully managed by StayBid</span>
          <span className="sbc2p-badge">⇄ B2B exchange</span>
        </div>
        ...
        <div className="sbc2p-h2">Choose your rooms</div>
        <p className="sbc2p-h2sub">Har room ka calendar kholo, apni check-in / check-out dates pick karo, aur uska market rate check karke bundle me add karo.</p>
      ...
      {count > 0 && (
        <div className="sbc2b-basket">
          <div className="sbc2b-basket-in">
            <span style={{ fontSize: ".78rem", opacity: .92 }}>{count} room-night set{count === 1 ? "" : "s"} · {inr(bTotal)}</span>
...
function RoomCard({ listing }: { listing: Listing }) {
  ...
  useEffect(() => {
    if (!key) { setInBasket(false); return; }
    const check = () => setInBasket(!!readBasket()[key]);
  ...
  const add = () => {
    if (!sel || !key) return;
    addItem({ key, listingId: listing.id, hotelId: listing.hotel_id, roomId: listing.room_id, title: f.title, city: f.city, room: f.room, image: f.roomImgs[0] || "", from, to, nights: sel.nights, buyPerNight: sel.buyPerNight, buyerPays: sel.buyerPays, marketAdr: sel.marketValuePerNight });
  };
    ...
    <div className="sbc2p-room">
      ...
      {open && (
        <div className="sbc2p-room-body">
          ...
          {/* market panel — buyer sees only their price vs the market (no internal rule) */}
          <div className="sbc2p-mkt">
            <div className="sbc2p-mkt-row"><span>Your buy price</span><b>{perN(f.buyN)}<small>/night</small></b></div>
            {q?.market ? (<>
              ...
              <div className="sbc2p-mkt-bar">
                ...
                <div className="sbc2p-mkt-track"><span className="sbc2p-mkt-dot" style={{ left: `${Math.max(3, Math.min(95, ((f.buyN - q.market.low) / Math.max(1, q.market.high - q.market.low)) * 100))}%` }} /></div>
                <span>high {perN(q.market.high)}</span>
              </div>
              {f.buyN < q.market.adr && <div className="sbc2p-mkt-note">📈 You buy {perN(q.market.adr - f.buyN)}/night below market — resale upside.</div>}
            </>) : <div className="sbc2p-mkt-note dim">Loading market price…</div>}
          </div>

          {/* clean check-in → check-out range picker (all released nights available) */}
          {win ? <RangeCalendar windowFrom={win.effectiveFrom} windowTo={win.to} blocked={blocked} from={from} to={to} onDay={onDay} onClear={clear} /> : <div className="sbc2p-mkt-note dim">Loading calendar…</div>}

          {from && to && sel && (
            <div className="sbc2p-selbox">
              <div className="sbc2p-sel-row"><span>{sel.nights} night{sel.nights === 1 ? "" : "s"} · {from} → {to}</span><b>{inr(sel.buyerPays)}</b></div>
              <div className="sbc2p-sel-sub">{inr(sel.subtotal)} + {inr(sel.buyerFee)} fee{sel.marketUpside > 0 ? <span className="sbc2p-upside"> · market value {inr(sel.marketValue)} → upside {inr(sel.marketUpside)}</span> : null}</div>
              <button className={inBasket ? "sbc-btn-ghost" : "sbc-btn-gold"} style={{ width: "100%", marginTop: 8 }} onClick={() => (inBasket ? removeItem(key) : add())}>{inBasket ? "✓ In bundle — remove" : `Add ${sel.nights} night${sel.nights === 1 ? "" : "s"} to bundle`}</button>
            </div>
          )}
          {from && !to && <div className="sbc2p-mkt-note">Ab apni check-out date pick karo.</div>}
        </div>
      )}
    </div>
  );
...
function RangeCalendar({ windowFrom, windowTo, blocked, from, to, onDay, onClear }: {
  windowFrom: string; windowTo: string; blocked: Set<string>; from: string; to: string; onDay: (d: string) => void; onClear: () => void;
}) {
  ...
  return (
    <div className="sbc2p-cal">
      <div className="sbc2p-cal-labels">
        <div className={`sbc2p-cal-lab${!from ? " active" : ""}`}><span>Check-in</span><b>{from || "select"}</b></div>
        <div className="sbc2p-cal-arrow">→</div>
        <div className={`sbc2p-cal-lab${from && !to ? " active" : ""}`}><span>Check-out</span><b>{to || "select"}</b></div>
        {(from || to) && <button className="sbc2p-cal-clear" onClick={onClear}>Clear</button>}
      </div>
      ...
      <div className="sbc2p-cal-grid">
        {grid.map((d, i) => {
          ...
          const cls = ["sbc2p-cal-day"];
          if (off) cls.push("off");
          if (isFrom) cls.push("from"); else if (isTo) cls.push("to"); else if (inRange(d)) cls.push("mid");
          return <button key={i} disabled={off} className={cls.join(" ")} onClick={() => onDay(d)}>{Number(d.slice(8))}</button>;
        })}
      </div>
    </div>
  );
...
function PropStyles() {
  return (
    <style jsx global>{`
      ...
      .sbc2p-hero { position: relative; height: 240px; border-radius: 18px; overflow: hidden; background: #e2d4bb; }
      ...
      .sbc2p-hero-cap { position: absolute; left: 0; right: 0; bottom: 0; padding: 30px 16px 14px; background: linear-gradient(0deg, rgba(20,14,7,.82), transparent); color: #fff; }
      .sbc2p-hero-title { font-size: 1.5rem; font-weight: 800; line-height: 1.1; }
      .sbc2p-hero-loc { font-size: .82rem; opacity: .9; margin-top: 2px; }
      .sbc2p-nav { position: absolute; top: 50%; transform: translateY(-50%); width: 36px; height: 36px; border-radius: 50%; border: 0; background: rgba(30,22,12,.5); color: #fff; font-size: 1.3rem; cursor: pointer; z-index: 2; }
      .sbc2p-nav.left { left: 8px; } .sbc2p-nav.right { right: 8px; }
      .sbc2p-nav.sm { width: 28px; height: 28px; font-size: 1.05rem; }
      .sbc2p-thumbs { display: flex; gap: 7px; overflow-x: auto; margin: 10px 0 0; padding-bottom: 4px; }
      .sbc2p-thumb { flex: none; width: 62px; height: 46px; border-radius: 9px; overflow: hidden; border: 2px solid transparent; padding: 0; background: none; cursor: pointer; }
      .sbc2p-thumb.on { border-color: var(--sbc-gold-deep); }
      .sbc2p-thumb img { width: 100%; height: 100%; object-fit: cover; }
      .sbc2p-badges { display: flex; flex-wrap: wrap; gap: 7px; margin: 14px 0 12px; }
      .sbc2p-badge { font-size: .72rem; font-weight: 700; color: var(--sbc-coffee); background: rgba(139,105,20,.09); border: 1px solid rgba(139,105,20,.16); border-radius: 999px; padding: 5px 11px; }
      .sbc2p-metrics { display: grid; grid-template-columns: repeat(2, 1fr); gap: 10px; margin: 4px 0 14px; }
      @media (min-width: 560px) { .sbc2p-metrics { grid-template-columns: repeat(4, 1fr); } }
      .sbc2p-metric { background: #fff; border: 1px solid rgba(139,105,20,.14); border-radius: 12px; padding: 12px 13px; }
      .sbc2p-metric b { display: block; color: var(--sbc-coffee); font-size: 1.05rem; font-weight: 800; }
      .sbc2p-metric span { font-size: .58rem; letter-spacing: .05em; color: rgba(74,56,32,.5); font-weight: 700; }
      ...
      .sbc2p-room { border: 1px solid rgba(139,105,20,.18); border-radius: 16px; margin-bottom: 12px; overflow: hidden; background: #fff; }
      ...
      .sbc2p-roomgal { position: relative; height: 160px; border-radius: 12px; overflow: hidden; margin: 11px 0; background: #e2d4bb; }
      ...
      .sbc2p-mkt { background: linear-gradient(135deg, #1f1710, #2c2116); color: #f3e7d0; border-radius: 13px; padding: 12px 14px; margin: 11px 0; }
      ...
      .sbc2p-cal { border: 1px solid rgba(139,105,20,.18); border-radius: 13px; padding: 11px; margin: 11px 0; background: #fffdf8; }
      .sbc2p-cal-labels { display: flex; align-items: center; gap: 8px; margin-bottom: 10px; }
      .sbc2p-cal-lab { flex: 1; border: 1px solid rgba(139,105,20,.2); border-radius: 10px; padding: 6px 10px; }
      .sbc2p-cal-lab.active { border-color: var(--sbc-gold-deep); background: rgba(212,162,74,.08); }
      .sbc2p-cal-lab span { display: block; font-size: .58rem; letter-spacing: .04em; color: rgba(74,56,32,.55); font-weight: 700; }
      .sbc2p-cal-lab b { font-size: .82rem; color: var(--sbc-coffee); }
      .sbc2p-cal-arrow { color: var(--sbc-gold-deep); font-weight: 800; }
      .sbc2p-cal-clear { border: 0; background: none; color: #c96f4a; font-size: .72rem; font-weight: 700; cursor: pointer; }
      .sbc2p-cal-head { display: flex; align-items: center; justify-content: space-between; font-weight: 800; color: var(--sbc-coffee); font-size: .88rem; margin-bottom: 8px; }
      .sbc2p-cal-head button { width: 30px; height: 30px; border-radius: 8px; border: 1px solid rgba(139,105,20,.25); background: #fff; color: var(--sbc-gold-deep); font-size: 1.05rem; cursor: pointer; }
      .sbc2p-cal-head button:disabled { opacity: .3; cursor: default; }
      .sbc2p-cal-dow { display: grid; grid-template-columns: repeat(7,1fr); gap: 3px; font-size: .6rem; color: rgba(74,56,32,.5); text-align: center; margin-bottom: 3px; font-weight: 700; }
      .sbc2p-cal-grid { display: grid; grid-template-columns: repeat(7,1fr); gap: 3px; }
      .sbc2p-cal-empty { aspect-ratio: 1; }
      .sbc2p-cal-day { aspect-ratio: 1; border: 0; border-radius: 9px; background: #fff; border: 1px solid rgba(139,105,20,.12); color: var(--sbc-coffee); font-size: .74rem; font-weight: 600; cursor: pointer; font-family: inherit; }
      .sbc2p-cal-day:hover:not(.off):not(.from):not(.to) { border-color: var(--sbc-gold-deep); }
      .sbc2p-cal-day.off { background: #f4efe4; color: rgba(74,56,32,.28); cursor: default; border-color: transparent; }
      .sbc2p-cal-day.mid { background: rgba(212,162,74,.24); border-color: transparent; border-radius: 4px; }
      .sbc2p-cal-day.from, .sbc2p-cal-day.to { background: var(--sbc-gold-deep); color: #fff; border-color: var(--sbc-gold-deep); }
      .sbc2p-cal-day.from { border-radius: 9px 4px 4px 9px; } .sbc2p-cal-day.to { border-radius: 4px 9px 9px 4px; }
